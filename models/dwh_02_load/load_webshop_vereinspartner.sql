{{ config(materialized="table", pre_hook=["{{ datavault_extension.refresh_external_table('DWS.EXT_WEBSHOP_VEREINSPARTNER','snowflake_external_table_surrogate') }}"], post_hook=["{{ datavault_extension.insert_hwm(this) }}"]) }}
